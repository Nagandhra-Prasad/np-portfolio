    ...
    <section id="experience" className="py-24">
      ...
      <div className="relative max-w-3xl mx-auto mb-16">
        ...
        {EXPERIENCES.map((exp, index) => (
          ...
          >
            <div className="absolute left-6 md:left-1/2 w-4 h-4 rounded-full bg-accent-purple border-4 timeline-ring -translate-x-1/2 mt-6 z-10 shadow-lg shadow-accent-purple/50" />
            ...
            <div className={`ml-14 md:ml-0 md:w-1/2 ${index % 2 === 0 ? "md:pr-12 md:text-right" : "md:pl-12"}`}>
              <div className="glass-hover p-6 rounded-2xl tilt-card text-left">
                <span className="inline-block px-3 py-1 text-xs font-bold rounded-full bg-accent-purple/20 accent-purple-text mb-3">
                  {exp.year}
                </span>
                <h3 className="text-xl font-display font-bold heading-text">{exp.role}</h3>
                <p className="text-accent-cyan text-sm font-medium mt-1">{exp.company}</p>
                <p className="body-text text-sm mt-3 leading-relaxed">{exp.description}</p>
                {exp.highlights && (
                  <ul className="mt-4 space-y-2">
                    {exp.highlights.map((point) => (
                      <li key={point} className="body-text text-xs leading-relaxed flex gap-2">
        ...
        {EDUCATION.map((edu, index) => (
          ...
          >
            <div className="absolute left-6 md:left-1/2 w-4 h-4 rounded-full bg-accent-cyan border-4 timeline-ring -translate-x-1/2 mt-6 z-10 shadow-lg shadow-accent-cyan/50" />
            ...
            <div className={`ml-14 md:ml-0 md:w-1/2 ${(EXPERIENCES.length + index) % 2 === 0 ? "md:pr-12 md:text-right" : "md:pl-12"}`}>
              <div className="glass-hover p-6 rounded-2xl tilt-card text-left">
                ...
                <h3 className="text-xl font-display font-bold heading-text">{edu.degree}</h3>
                <p className="text-accent-purple text-sm font-medium mt-1">{edu.institution}</p>
                <p className="body-text text-sm mt-3">{edu.grade}</p>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      >
        <h3 className="text-2xl font-display font-bold text-center heading-text mb-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {ACHIEVEMENTS.map((achievement) => (
            <div key={achievement} className="glass p-4 rounded-xl flex gap-3 items-start">
              ...
              <p className="subtle-text text-sm leading-relaxed">{achievement}</p>
            </div>
          ))}
        </div>
      </motion.div>
